...
import (
	...
	"user/pkg/errors"
	"user/pkg/healthz"
	jaegerPkg "user/pkg/jaeger"
	kafkaPkg "user/pkg/kafka"
	"user/pkg/limiter"
	loggerPkg "user/pkg/logger"
	redisPkg "user/pkg/redis"
	"user/pkg/token"
	"user/pkg/utils"
...
func main() {
	...
	g.Go(func() error {
		return userServer.RunMetrics(cancel)
	})

	g.Go(func() error {
		return userServer.RunGateway()
	})

	g.Go(func() error {
		return userServer.Run()
	})
